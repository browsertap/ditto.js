...
BaseController.extend(ConsoleController, {
  ...
  _log: function (level, args) {
    ...
    this.scope.emit("log", this.label, level, args, this.hotswap._id);
  ...
  _onLog: function (platform, level, args, hotswapId) {

    if (this.hotswap._id == hotswapId) return;

    this._ignore = true;
    args[0] = platform + ": " + args[0];
    console[level].call(console, msg, args);
    this._ignore = false;
  }
});
